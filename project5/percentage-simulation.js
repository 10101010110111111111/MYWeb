class PercentageSimulationManager {
  ...
  updateQuickOptionButtons(activeValue) {
    document.querySelectorAll(".quick-option-btn").forEach((btn) => {
      ...
      if (Number.parseInt(btn.dataset.value) === activeValue) {
  ...
  updateConfigurationDisplay() {
    ...
    const shufflePoint = (Number.parseFloat(this.elements.percentageSimShuffle.value) * 100).toFixed(0)
    const countingSystem =
      this.elements.percentageSimCounting.options[this.elements.percentageSimCounting.selectedIndex].text
  ...
  startSimulation() {
    ...
    const handsToSimulate = Number.parseInt(this.elements.percentageSimHands.value)
  ...
  initializeDeck() {
    const deckCount = Number.parseInt(this.elements.percentageSimDecks.value)
    const shufflePoint = Number.parseFloat(this.elements.percentageSimShuffle.value)
    ...
    // OPRAVENO: Inicializace running count pro KO systém
    ...
    if (countingSystem === "ko") {
      ...
      this.runningCount = this.getKOInitialCount(deckCount)
    } else {
      this.runningCount = 0
    }
  }
  ...
  simulateSingleHand() {
    ...
    // OPRAVENO: Použijeme původní délku balíčku místo aktuální délky
    const totalCards = this.currentDeck.length + this.cardsDealt
    const remainingCards = totalCards - this.cardsDealt
    ...
    if (this.cardsDealt >= this.shufflePoint) {
      this.shuffleDeck()
      this.cardsDealt = 0
      // OPRAVENO: Pro nevyvážené systémy (KO) nerestujeme running count
      const countingSystem = this.elements.percentageSimCounting.value
      if (countingSystem !== "ko") {
        this.runningCount = 0 // Reset running count when shuffling for balanced systems
      }
    }

    return {
      trueCount: Math.max(-10, Math.min(10, Math.floor(trueCount))), // Use floor for consistent TC calculation
  ...
  shuffleDeck(deck = null) {
    if (deck) {
      ...
      // OPRAVENO: Inicializace running count při shuffle
      const countingSystem = this.elements.percentageSimCounting.value
      if (countingSystem === "ko") {
        this.runningCount = this.getKOInitialCount(deckCount)
      } else {
  ...
  // OPRAVENO: Přidána funkce pro KO systém počáteční count
  getKOInitialCount(deckCount) {
    const initialCounts = {
      1: 0,
      2: -4,
      6: -20,
  ...
  playHand(playerCards, dealerCards, trueCount) {
    ...
    const tc = Math.floor(trueCount) // Use floor for consistency
    ...
    if (playerBJ && dealerBJ) {
      return { result: "push", bet: 1, isBlackjack: false, isDouble: false, insuranceResult: 0 }
    }
    if (playerBJ && !dealerBJ) {
      return { result: "blackjack", bet: 1, isBlackjack: true, isDouble: false, insuranceResult: 0 }
    }
    if (!playerBJ && dealerBJ) {
      return { result: "loss", bet: 1, isBlackjack: false, isDouble: false, insuranceResult: 0 }
    }

    // Handle insurance if dealer shows Ace
    let insuranceResult = 0
    if (dealerCards[0].value === "A") {
      const insuranceBet = this.handleInsurance(trueCount, strategy)
      if (insuranceBet > 0) {
        // Insurance pays 2:1 if dealer has blackjack
        if (dealerBJ) {
          insuranceResult = insuranceBet * 2 // +2 units (insurance bet is 1 unit)
        } else {
          insuranceResult = -insuranceBet // -1 unit
        }
      }
    }
    ...
    return { result: "push", bet: currentBet, isBlackjack: false, isDouble: isDouble, insuranceResult: insuranceResult }
  }
  ...
  playSplitHand(playerCards, dealerCards, trueCount) {
    ...
    // Play hand 1
    const result1 = this.playSingleSplitHand(hand1, dealerCards, tc, strategy)
    // Play hand 2
    const result2 = this.playSingleSplitHand(hand2, dealerCards, tc, strategy)
  ...
  playSingleSplitHand(playerCards, dealerCards, tc, strategy) {
    let currentBet = 1
    let isDouble = false
    let playerValue = this.calculateHandValue(playerCards)

    // For split aces, typically only one card is dealt and no further action allowed
    ...
    // Check for bust
    if (playerValue > 21) {
      return { result: "loss", bet: currentBet, isBlackjack: false, isDouble: isDouble, insuranceResult: 0 }
    }

    // Check for 21 (but not blackjack in split hands)
    const is21 = playerValue === 21

    // Play dealer hand (reuse existing dealer cards)
    let dealerValue = this.calculateHandValue(dealerCards)
    while (dealerValue < 17) {
      const newCard = this.dealCard()
      dealerCards.push(newCard)
    ...
    // Determine result
    if (dealerValue > 21) {
      return { result: "win", bet: currentBet, isBlackjack: false, isDouble: isDouble, insuranceResult: 0 }
    }
    if (playerValue > dealerValue) {
      return { result: "win", bet: currentBet, isBlackjack: false, isDouble: isDouble, insuranceResult: 0 }
    }
    ...
    return { result: "push", bet: currentBet, isBlackjack: false, isDouble: isDouble, insuranceResult: 0 }
  }
  ...
  getSplitStrategy(playerCards, dealerValue) {
    const cardValue = this.getCardValue(playerCards[0])
    
    // Split strategy according to specifications
    switch (cardValue) {
      case 11: // A,A
      ...
      case 9: // 9,9
        ...
        if (dealerValue === 10 || dealerValue === 11) return "stand"
        return "stand"
  ...
  getDeviations(playerCards, dealerValue, trueCount) {
    const playerValue = this.calculateHandValue(playerCards)
    const isSoft = this.isSoftHand(playerCards)
    const tc = Math.floor(trueCount) // Use floor for consistent TC calculation

    // Illustrious 18 deviations (complete)
    if (!isSoft) {
      ...
      if (playerValue === 12 && dealerValue === 4 && tc >= 0) return "stand"

      // 12 vs 5: Stand at TC >= -2
      if (playerValue === 12 && dealerValue === 5 && tc >= -2) return "stand"

      // 12 vs 6: Stand at TC >= -1
      if (playerValue === 12 && dealerValue === 6 && tc >= -1) return "stand"
      ...
      // 8 vs 6: Double at TC >= 0
      if (playerValue === 8 && dealerValue === 6 && tc >= 0) return "double"
    }

    // Soft hand deviations
    if (isSoft) {
      // A,8 vs 6: Double at TC >= 0
      if (playerValue === 19 && dealerValue === 6 && tc >= 0) return "double"

      // A,7 vs 2: Double at TC >= 0
      if (playerValue === 18 && dealerValue === 2 && tc >= 0) return "double"

      // A,7 vs 7: Double at TC >= 3
      if (playerValue === 18 && dealerValue === 7 && tc >= 3) return "double"

      // A,6 vs 2: Double at TC >= 1
      if (playerValue === 17 && dealerValue === 2 && tc >= 1) return "double"
    }
  ...
  processHandResult(result) {
    ...
    if (handResult.result === "split") {
      ...
      for (const hand of handResult.hands) {
        this.recordResult(trueCount, hand.result, hand.bet, hand.isBlackjack, hand.isDouble, hand.insuranceResult)
      }
    } else {
      // Process single hand
      this.recordResult(
  ...
  recordResult(trueCount, result, initialBet = 1, isBlackjack = false, isDouble = false, insuranceResult = 0) {
    const tc = Math.max(-10, Math.min(10, Math.floor(trueCount))) // Clamp TC to valid range
    if (tc >= -10 && tc <= 10) {
      ...
      if (result === "blackjack") {
        ...
        // Blackjack pays 3:2, so net gain is +1.5 units (you get back bet + 1.5x bet)
        netUnits = initialBet * 1.5
      } else if (result === "win") {
        this.results[tc].wins++
        // Regular win pays 1:1, so net gain is +1 unit (you get back bet + equal amount)
        netUnits = initialBet * 1.0
      } else if (result === "loss") {
        this.results[tc].losses++
        // Loss means you lose your bet, so net loss is -1 unit
        netUnits = -initialBet * 1.0
      } else if (result === "push") {
        this.results[tc].pushes++
        // Push means you get back your bet, so net gain is 0
        netUnits = 0
      }

      // No additional multiplication needed since initialBet already reflects the doubled amount

      // Add insurance result (already calculated correctly)
      netUnits += insuranceResult

      // Update EV tracking
      this.results[tc].totalNetUnits += netUnits
      this.results[tc].totalInitialBets += initialBet

      // Calculate current EV: EV = (totalNetUnits / totalInitialBets) * 100%
